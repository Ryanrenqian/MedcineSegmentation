...
#         print('dpts:',time.time()-time1)
    ...
    def finalprobmap(self,slide_path, roi_path=None, max_k=82,save=None,num_worker=None):
        ...
        print(lkh,lkw)
        skip_w = 1 # 是否要计算x的最后一块
        skip_h = 1 # 是否要计算y的最后一块
        # 初始化fpm的大小
        if lw-260<0:
            x = 2*(kw * (max_k+1)) 
            skip_w = 0
...
#                 if dpt.shape != fpm[flag_x:flag_x+flag_x_w, flag_y:flag_y+flag_y_h].shape:
#                     pdb.set_trace()
                fpm[flag_x:flag_x+flag_x_w, flag_y:flag_y+flag_y_h] = dpt
                time4 = time.time()
