...
class Hard(BasicHard):
    ...
    def load_data(self):
        ...
        test_dataset = ListDataset(self.config.get_config('train', 'normal_list'),
    ...
    def hard(self, _model, epoch, save_helper):
        '''
        动态的加载模型用于训练
        Parameters
        ----------
        ...
        print('loading test json list complete! time:%0.4f  total patch:%d' % (
        (time.time() - load_time), len(patch_dict)))
        ...
        for slide_index in range(start_index, end_index):
            ...
            for i, data in enumerate(dataloader, 0):
                ...
                acc_batch_total, acc_batch_pos, acc_batch_neg = accuracy.acc_binary_class(test_output.cpu(),
                                                                                          batch_labels, 0.5)
                ...
                accuracy.handle_binary_classification(test_output.cpu(), batch_labels, path_list,
                                                      acc['epoch_acc_image'], 0.5)
            ...
            save_helper.save_epoch_pred(acc['epoch_acc_image'],
                                        f'hardmine_epoch_{epoch}_slide_{slide_name}.txt')
            save_helper.save_epoch_model(epoch, 'hard', acc, None, _model)
